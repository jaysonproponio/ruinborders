...
$query = "SELECT a.*, ad.fullname as admin_name FROM announcements a 
...
$query = "SELECT * FROM payment_receipts WHERE user_id = :user_id AND status = 'pending' ORDER BY created_at DESC";
$stmt = $db->prepare($query);
$stmt->bindParam(':user_id', $user_id);
$stmt->execute();
...
<head>
    ...
    <title>Dashboard - Ruin Borders</title>
    ...
    <div class="main-content">
        ...
        <?php if (isset($_SESSION['login_success'])): ?>
            <div style="background: #d4edda; color: #155724; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #28a745;">
                ...
                <?php if (isset($_SESSION['login_time'])): ?>
                    <small style="display: block; margin-top: 5px; opacity: 0.8;">Logged in at <?php echo htmlspecialchars($_SESSION['login_time']); ?></small>
                <?php endif; ?>
            </div>
            <?php 
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo count($pending_receipts); ?></div>
        ...
        <div class="content-grid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Payment Status - <?php echo $current_year; ?></h3>
                    <a href="payments.php" class="btn btn-primary">
                        <i class="fas fa-eye"></i> View Details
                    </a>
                </div>
                <div class="payment-grid">
                    <?php foreach ($payment_status as $index => $status): ?>
                        <div class="payment-month <?php echo $status['is_paid'] ? 'paid' : 'pending'; ?>">
                            <div class="month-name"><?php echo $status['month']; ?></div>
                            <div class="month-amount">
                                <?php echo $status['is_paid'] ? '₱' . number_format($status['amount'], 2) : 'Not Paid'; ?>
                            </div>
                            <div class="month-status <?php echo $status['is_paid'] ? 'paid' : 'pending'; ?>">
                                <?php echo $status['is_paid'] ? 'Paid' : 'Pending'; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                    ...
                    <?php foreach ($announcements as $announcement): ?>
                        <a href="announcements.php" class="announcement-link">
                            <div class="announcement-item">
                                ...
                                <div class="announcement-content"><?php echo htmlspecialchars(substr($announcement['content'], 0, 100)) . '...'; ?></div>
                                <div class="announcement-meta">
                                    By <?php echo htmlspecialchars($announcement['admin_name']); ?> • 
